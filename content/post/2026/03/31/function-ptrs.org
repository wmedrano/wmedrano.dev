...
Spoiler: I don't know. For my adhoc interpreter, function pointers were a bit
slower, but more flexible. See the [[id:89620a29-0426-4063-8f03-fa8a5c7ba15d][Results section]].
...
bytes~. OCaml's dynamic value type is stored as either a [[https://blog.janestreet.com/what-is-gained-and-lost-with-63-bit-integers/][=63bit int= or a
pointer]].
...
- They take more bytes to represent. The ~enum~ used in this interpreter uses =2
  ...
  byte. This 4.5x size difference means fewer instructions fit in CPU cache,
...
- Dispatching an enum should have lower overhead than even a tail function
  ...
  this, the impact of the dispatch mechanism may change as the complexity of the
  instructions grows.
...
#+BEGIN_SRC gnuplot :file results-add.svg :exports results
set terminal svg size 600,300 background "#1e1e2e" font "Inter,13"
set style data histogram
set style fill solid 0.8 border -1
set boxwidth 0.6
set yrange [0:0.07]
set ylabel "microseconds" textcolor "#cdd6f4"
set title "add(1+2, 3+4) benchmark" textcolor "#cdd6f4"
set border lc rgb "#cdd6f4"
set tics textcolor "#cdd6f4"
set key off
...
"Fn Ptr Hyper" 0.0448
EOD
plot $data using 2:xtic(1) lc rgb "#89b4fa"
#+END_SRC

#+RESULTS:
[[file:results-add.svg]]

#+BEGIN_SRC gnuplot :file results-fib.svg :exports results
set terminal svg size 600,300 background "#1e1e2e" font "Inter,13"
set style data histogram
set style fill solid 0.8 border -1
set boxwidth 0.6
set yrange [0:15]
set ylabel "microseconds" textcolor "#cdd6f4"
set title "fib(12) benchmark" textcolor "#cdd6f4"
set border lc rgb "#cdd6f4"
set tics textcolor "#cdd6f4"
set key off
set xtics rotate by -20
...
Two microbenchmarks aren't enough to characterize a dispatch mechanism. Results
may look very different with a more diverse or realistic workload.
...
pipeline — the same sequence of operations applied repeatedly across batches of
pixels.
