...
series. In parts [[file:~/src/wmedrano.dev/content/post/2025/11/15/lisp-1.org][1]] and [[file:~/src/wmedrano.dev/content/post/2026/01/25/lisp-2.org][2]], we implemented the parser and ~eval~.
...
#+BEGIN_SRC dot :file env.png :exports results
digraph {
  graph [ dpi = 300 ];
  bgcolor=transparent;
  node[shape=record, fontname=Sen];
...
#+BEGIN_SRC dot :file env2.png :exports results
digraph {
    graph [ dpi = 300 ];
    bgcolor=transparent;
    node[shape=record, fontname=Sen];
