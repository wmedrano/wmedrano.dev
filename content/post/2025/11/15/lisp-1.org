...
This is Part 1 (Parsing) of a 3-part series.
Reading: [[file:../../../2026/01/25/lisp-2.org][Part 2 (Evaluation)]] | [[file:../../../2026/02/21/lisp-3.org][Part 3 (Lambdas)]]

* Part 1: Parsing

** Why Lisp

Implementing a compiler or interpreter may seem like a daunting task, but a
Lisp's simplicity makes it an easy starting point.

Evaluating a Lisp program boils down to two steps:

1. ~read~ - Read the text into a Lisp data structure.
2. ~eval~ - Evaluate that structure according to Lisp’s simple rules.

#+BEGIN_SRC scheme
12 ;; An integer, 12. (eval (read "12"))    => 12
#true ;; A boolean    (eval (read "#true")) => #true
...
the type of a variable is determined at runtime. The type may also change. In
the following example, observe how the type of ~x~ freely changes.
...
We are going to represent lists as simple ~Vec<T>~ since Rust deals with them
better. However, we should discuss how traditional Lisps represent their most
...
as ~Vec<Val>~. Rust is much better at dealing with slices than (infamously)
linked lists.
...
#+BEGIN_SRC rust
#[test]
fn tokenizer_separates_by_token() {
...
#+BEGIN_SRC rust
#[test]
fn read_returns_lisp_datastructure() {
...
#[test]
fn read_unmatched_paren_returns_error() {
...
#[test]
fn read_extra_closing_paren_returns_error() {
